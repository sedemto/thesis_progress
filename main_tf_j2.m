...
paramsolver.lambda = 0.001;%0.0000001;  % threshold (regularization parameter), potential candidate 0.01 or 0.001
...
%         figure, imagesc(angle(gapped_spec)), colorbar, axis xy
        ...
        for gap=1:5 % iterate gaps
            ...
            % make sure the first index-1 of segment_indx is divisible by 4
            pad = 4;
...
%             segment.data = FDGT(new_data, tight_win, sigIdx2, FFTnum, rotIdx2, zeroPhaseFlag);
 %%%%
            ...
            % precomputation for fast DGT
            zeroPhaseFlag = true;
            [sigIdx2, sumIdx2, sumArray2, ifftArray2, rotIdx2] = precomputationForFDGT(size(segment.gapped,2)*shiftLen, param.w, param.a, param.M);
            new_G_adj =@(u) invFDGT(u, tight_win, sumIdx2, sumArray2, ifftArray2, rotIdx2, zeroPhaseFlag)*winLen;
            new_G = @(x) FDGT(x, tight_win, sigIdx2, FFTnum, rotIdx2, zeroPhaseFlag);
            segment.gapped_signal = new_G_adj(segment.gapped);
            signal_max = max(segment.gapped_signal);
            fprintf('U-PHAIN...gap number: %d\n',gap)
            
            param.type = 'U';
            paramsolver.I =500;
            paramsolver.J = 10;
            [segment.solution] = PHAINmain_TF_GCPA(segment.gapped_signal,segment.gapped, segment.mask, param, paramsolver,segment.data);
...
%             segment.solution = new_G(segment.solution);
